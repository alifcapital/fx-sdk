...
> **Точность десятичных:** `Quantity`, `LimitRate` и `MinTradeQuantity` несут не
> более **6 знаков после запятой** — столько хранят и SDK, и Core. Значение с
> большей точностью отклоняется с `v1.ErrDecimalPrecision` ещё до записи в
> локальную БД и до отправки в Core, а не округляется молча: сумма, под которую
> партнёр зарезервировал средства, должна совпадать с суммой сделки. Хвостовые
> нули точностью не считаются — `"1000.1000000"` принимается.
...
Порядко-независимая XOR-контрольная сумма по **всему кортежу** сделки и число
строк, которое она покрывает:

```sql
bit_xor(hashtextextended(
    trade_id || ':' || order_id || ':' || side || ':' ||
    filled_quantity::text || ':' || execution_rate::text, 0)),
count(*)
```

Обе стороны считают одно и то же выражение — Core по своей таблице расчётов, SDK
по `client_trades`. Равные сумма **и** количество означают, что совпадают и
множества сделок, и их значения.

> **Почему по кортежу, а не по идентификаторам.** Раньше суммировалось
> `bit_xor(hashint8(trade_id) # hashint8(order_id))`, и у этого было три слепых
> пятна. Строка, где `trade_id` равен `order_id`, давала в XOR ноль — потерять её
> было невозможно заметить, а идентификаторы это две независимые
> последовательности от единицы, так что на малых номерах равенство обычное дело.
> Любые две строки с одинаковым вкладом гасили друг друга — например пара
> `(a,b)` и `(b,a)`. И в хэш не входило ничего, кроме идентификаторов. Количество
> строк передаётся отдельно, потому что пустой час и час, где вклады
> сократились, дают один и тот же ноль.
>
> Против Core, который не присылает новые поля, используется прежняя сумма;
> `ReconcileResult.HashVersion` показывает, какое сравнение выполнялось.
...
Расхождение в объёме или курсе сумма теперь видит, но не говорит, **в какой
именно** сделке — для этого по-прежнему нужен `ReconcileDiff`.
...
|------------------------|----------|
| `LocalHash` / `RemoteHash` | Контрольные суммы двух сторон — те, которые реально сравнивались |
| `HashVersion` | `2` — сумма по кортежу, `1` — старая сумма по идентификаторам (Core не прислал новые поля) |
| `LocalTrades` / `RemoteTrades` | Сколько сделок вошло в сумму с каждой стороны. `RemoteTrades = -1` означает, что Core количество не прислал |
