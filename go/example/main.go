...
func main() {
	var (
		target    = flag.String("target", "192.168.97.5:80", "FX Core gRPC address")
		...
		clientID  = flag.String("client-id", "1253", "client identifier (required for filter)")
		clientINN = flag.String("client-inn", "07128303", "client INN (taxpayer ID)")
	...
	g.Go(func() error {
		...
		err := client.SubscribeOrderEvents(ctx, func(event *v1.OrderEvent) {
	...
	// settlement and fee, and updates the parent order; the handler below is
	// responsible for the partner-specific account movements.
	g.Go(func() error {
		log.Printf("starting trade subscription...")
		err := client.SubscribeTrades(ctx, func(ctx context.Context, ev *v1.TradeEvent) error {
	...
	// account details
	acc["debit_account"] = "1253"
	acc["credit_account"] = "4553"
	...
	default:
		log.Printf("submitted: ref_id=%d order_id=%d status=%d cause=%q",
	...
	if *cancel && submitted != nil && submitted.OrderID != 0 {
